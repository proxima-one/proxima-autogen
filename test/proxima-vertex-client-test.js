...
describe('proxima vertex client gen', async function() {
  ...
  it('should run correctly', async function() {
    ...
    autogen.generateProximaVertexClient(config)
    assert(true);
  });
  //correct file structure
  it('should have the correct file structure', function() {
    assert(fs.pathExistsSync("./vertex-client/"), true)
    assert(fs.pathExistsSync("./vertex-client/index.js"), true)
  });
  //exports done correctly
  it('should export the vertex correctly with queries', function() {
    assert(fs.pathExistsSync("./vertex-client/app_queries.json"), true)
    cleanup();
    assert(true);
  });
});
