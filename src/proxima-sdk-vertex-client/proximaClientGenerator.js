...
const clientFileTemplate = fs
  .readFileSync(require.resolve("./proximaVertexClientTemplate.js"))
  .toString();

function generateVertexClient(config) {
  let clientFileName = "./proxima-sdk-vertex-client/index.js";
  let configText = generateConfigText(config);
  let clientFileText = clientFileTemplate.replace(
    "//CONFIG CODE TO BE ADDED",
    configText
  );
  fs.outputFileSync(clientFileName, clientFileText);

  let queryFileName = "./proxima-sdk-vertex-client/app_queries.json";
...
function generateTestGetAllQuery(entityName, entityText) {
  let entityGetAllQueryString =
    "query { " +
    entityName +
    "s(first: $first , last: $last, limit: $limit, prove: $prove) {" +
    entityText +
    "}}";
...
function generateGetAllQuery(entityName, entityText) {
  let entityGetAllQueryString =
    ...
    " gql'" +
    entityName +
    "s($first: Int, $last: Int, $limit: Int, $prove: Boolean) { \n" +
    entityName +
    "s(first: $first , last: $last, limit: $limit, prove: $prove) { \n" +
    entityText +
    "\n}\n}'\n\n";
...
function generateSearchQuery(entityName, entityText) {
  let entitySearchQueryString =
    " gql'" +
    entityName +
    "s($queryText: Text!, $prove: Boolean) {\n" +
    entityName +
    "s(queryText: $queryText, prove: $prove) {\n" +
    entityText +
    "\n}\n}'\n\n";
  return entitySearchQueryString;
}
